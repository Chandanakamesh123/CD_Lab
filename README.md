...
FLEX scans the input and matches it against **regular expressions**. Whenever an identifier is recognized, it is inserted into the symbol table.
...
A lexical analyzer divides the source program into **tokens**. Each token is identified using a regular expression defined in the FLEX program.
...
FLEX can include this header file so that it can return the correct tokens.
...
If you have limited preparation time, prioritize the experiments in this order:
...
These six experiments cover the main **FLEX/BISON programming patterns** used in the first part of the Compiler Design Laboratory.
...
These programs are intended for **Compiler Design Laboratory practical preparation** and demonstrate the basic use of **FLEX for lexical analysis** and **BISON for syntax analysis and expression evaluation**.
